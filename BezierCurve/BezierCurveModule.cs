...
namespace BezierCurve
{
    public partial class BezierCurveModule : Module
    {
        ...
        private void actBZCurve_Execute(object sender, ExecuteEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < line.Count-2; i++)
                {
                    ...
                    var nodeS = line[i];
                    var pointS = surface.Points[nodeS.Index];
                    var nodeF = line[i+1];
                    var pointF = surface.Points[nodeF.Index];
                    var nodeN = line[i+2];
                    var pointN = surface.Points[nodeN.Index];
                    ...
                    double dy = LS * coef * Math.Cos(C1);
                    double dx = LS * coef * Math.Sin(C1);

                    var vectorFC1 = new Vector3D(pointF.Vertex.X + dx, pointF.Vertex.Y + dy, pointF.Vertex.Z);

                    dy = LF * coef * Math.Cos(C2);
                    dx = LF * coef * Math.Sin(C2);
                    ...
                    segments[i, 3] = segments[i + 1, 0] = pointF;
                ...
                {
                    double t = 0.0;
                    int N = 10;//количество шагов кривой
        ...
        double DirAngle(SurfacePoint begin, SurfacePoint end)
        {
